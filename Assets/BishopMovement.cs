...
public class BishopMovement : MonoBehaviour
{
    ...
    private static readonly Vector3[] BishopOffsets = new Vector3[]
    {
        new Vector3( 1, 0,  1),
        new Vector3( 2, 0,  2),
        new Vector3( 3, 0,  3),
    ...
    {
        Vector3[] moves = GenerateMoves(BishopOffsets);
        ...
        if (player != null)
        {
            ...
            foreach (var move in moves)
            {
                ...
                {
                    Debug.Log("GONIEC BIJE GRACZA NATYCHMIAST (ignore turns)");
                    Move(move);
                    return;
                }
            }
    ...
    private Vector3 SelectMove(Vector3[] possibleMoves)
    {
        ...
        for (int i = 0; i < possibleMoves.Length; i++)
        {
            ...
            points[i] = -distance; // im bli¿ej gracza, tym lepiej
    ...
    private void Move(Vector3 newPos)
    {
        ...
        {
            Destroy(player);
            Debug.Log("GONIEC WYGRA£ – GRACZ ZOSTA£ ZABITY");
        }
